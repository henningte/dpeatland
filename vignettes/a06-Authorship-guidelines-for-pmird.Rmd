---
title: "Authorship guidelines for dpeatland"
output: 
  html_document:
    toc: true
...
This document is intended to foster an open dialog on authorship to the data compendium dpeatland. Note that there are separate [authorship guidelines for the data paper on dpeatland](a07-Authorship-guidelines-for-the-data-paper.html). The document is based on the authorship guidelines template document developed by @Oliver.2018.

In the table below, all tasks for the succesfull development of dpeatland are listed. In the right column, the names of all current contributors are listed for the respective task. This document is continuously updated if new data contributors provide data and you can check if your contribution is appropriately acknowledged. 

The information provided in this table are the basis for the authorship order and the description of inidividual contributions to dpeatland according to the guidelines listed below.

If you feel that you are not appropriately acknowledged or that some of the guidelines are not acceptable for you, contact henning.teickner@uni-muenster.de or open an [issue](https://github.com/henningte/dpeatland/issues) and describe your problem. 

## Activities

```{r activities}
rbind( 
  c("a) Conceived the idea/concept for the data compendium", "Henning Teicker"),
  c("b) Designed the data compendium", "Henning Teicker"),
  c("c) Managed and supervised data contributions and data collection", "Henning Teicker"),
  #
  c("d) Contributed or synthesised data (this also includes a textual interpretation/evaluation of the data and data quality)", ""),
  c("e) Developed code for managing and accessing the data compendium", ""),
  #
  c("f) Supervised the project and data compendium development", ""),
  # 
  c("g) Other contributions not listed above (e.g., person has a light-bulb moment that completely changes scope/slant of project), please specify", "")
  ) %>%
  as.data.frame() %>%
  ...
  kableExtra::pack_rows("Category 4: OTHER", 7, 7)
